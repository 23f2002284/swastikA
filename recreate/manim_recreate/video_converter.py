...
def render_kolam(
    ...
    *,
    ...
    cfg = {
        ...
        "write_to_movie": not bool(save_last_frame),
...
def render_kolam_node(state: RenderState) -> RenderState:
    ...
    movie_path, image_path = render_kolam(
        svg_path=svg_path,
...
def build_graph():
    """Build and return a LangGraph StateGraph for rendering kolam animations."""
    sg = StateGraph(RenderState)
    sg.add_node("render_kolam", render_kolam_node)
    sg.set_entry_point("render_kolam")
    sg.add_edge("render_kolam", END)
    return sg.compile()
