# inspired or more like copied from https://github.com/Yanick112/ComfyUI-ToSVG
import vtracer
import os
import time
import os
...
class SaveImage:
    ...
    OUTPUT_NODE = True
    CATEGORY = "image"
    DESCRIPTION = "Saves the input images to the output directory."
    ...
    def save_images(self, images, filename_prefix="ComfyUI", prompt=None, extra_pnginfo=None):
        ...
        filename = f"{filename_prefix}_{int(datetime.now().timestamp())}.png"
        file_path = os.path.join(self.output_dir, filename)
        
        # Convert tensor to PIL Image and save
        image = 255.0 * images[0].cpu().numpy()
        image = Image.fromarray(np.clip(image, 0, 255).astype(np.uint8))
        image.save(file_path, compress_level=self.compress_level)
        ...
        return {"ui": {"images": [{"filename": filename, "subfolder": "", "type": self.type}]}}
import re
